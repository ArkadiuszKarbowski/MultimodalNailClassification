...
def count_nails_in_folder(folder_path):
    ...
    if invalid_subfolders:
        print(
            f"Invalid subfolders in '{os.path.basename(folder_path)}': {', '.join(invalid_subfolders)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Count nails in subfolders based on subfolder names."
    )
    parser.add_argument(
        "folder_path",
        type=str,
        help="Path to the folder to search recursively (depth 1).",
    )
