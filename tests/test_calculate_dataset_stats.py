...
def test_calculate_dataset_statistics(mocker):
    future1 = Future()
    future1.set_result([
        ('path/to/UV_img1.jpg', np.array([0.1, 0.2, 0.3]), np.array([0.4, 0.5, 0.6])),
        ('path/to/UV_img2.jpg', np.array([0.2, 0.3, 0.4]), np.array([0.5, 0.6, 0.7]))
    ])
    
    future2 = Future()
    future2.set_result([
        ('path/to/normal_img1.jpg', np.array([0.7, 0.8, 0.9]), np.array([0.2, 0.3, 0.4])),
        ('path/to/normal_img2.jpg', np.array([0.6, 0.7, 0.8]), np.array([0.1, 0.2, 0.3]))
    ])
    
    mock_executor = mocker.patch('src.utils.calculate_dataset_stats.ProcessPoolExecutor')
    mock_executor_instance = mock_executor.return_value.__enter__.return_value
    mock_executor_instance.submit.side_effect = lambda func, batch: future1 if 'UV' in batch[0] else future2
    
    mocker.patch('src.utils.calculate_dataset_stats.as_completed', side_effect=lambda fs: fs)
    
    stats = calculate_dataset_statistics([
        'path/to/UV_img1.jpg', 'path/to/UV_img2.jpg',
        'path/to/normal_img1.jpg', 'path/to/normal_img2.jpg'
    ], batch_size=2)
