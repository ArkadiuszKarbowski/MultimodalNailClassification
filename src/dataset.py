...
def prepare_dataset(root_dir, img_size=224, batch_size=32):
    ...
    train_nail_ids, test_nail_ids = train_test_split(
        nail_ids, test_size=0.2, random_state=42, stratify=class_labels
    )

    train_paths = [
        path for nail_id in train_nail_ids for path in nail_groups[nail_id]["paths"]
    ]
    test_paths = [
        path for nail_id in test_nail_ids for path in nail_groups[nail_id]["paths"]
    ]

    train_dataset = NailDataset(
        train_paths,
        transform=transform_uv if "UV" in train_paths[0] else transform_normal,
    )
    test_dataset = NailDataset(
        test_paths,
        transform=transform_uv if "UV" in test_paths[0] else transform_normal,
    )
